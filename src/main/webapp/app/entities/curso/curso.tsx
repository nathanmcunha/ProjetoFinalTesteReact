...
export class Curso extends React.Component<ICursoProps> {
  ...
  render() {
    ...
    return (
      <div>
        <h2 id="curso-heading">
          <Translate contentKey="projetoFinalTesteReactApp.curso.home.title">Cursos</Translate>
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />&nbsp;
            <Translate contentKey="projetoFinalTesteReactApp.curso.home.createLabel">Create new Curso</Translate>
          </Link>
        </h2>
            ...
            <tbody>
              {cursoList.map((curso, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    {curso.disciplinas
                      ? curso.disciplinas.map((val, j) => (
                          <span key={j}>
                            <Link to={`disciplina/${val.id}`}>{val.id}</Link>
                            {j === curso.disciplinas.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
